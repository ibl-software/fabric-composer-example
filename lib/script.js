...
async function tradeCommodity(transfer) { // eslint-disable-line no-unused-vars
  ...
  if((transfer.Ownership.lOwner.propertyId.unpaidTaxes == 0 || 
     transfer.Ownership.nOwner.propertyId.unpaidTaxes == 0 ) &&
     (transfer.Ownership.lOwner.propertyId.bank.value == 0||
     transfer.Ownership.nOwner.propertyId.bank.value == 0) &&
     (transfer.Ownership.lOwner.propertyId.stat == "transferalbe" ||
     transfer.Ownership.nOwner.propertyId.stat == "transferalbe") &&
     (transfer.Ownership.lOwner.legOwner.judgStat == "verified" ||
     transfer.Ownership.nOwner.natOwner.judgStat == "verified") &&
      (transfer.Ownership.lOwner.legOwner.RegStat == "verified" ||
     transfer.Ownership.nOwner.natOwner.RegStat == "verified"))
    {
...
//   }
//   else{
// 	throw new Error('the property or the Owner used facilies before!!');  
//   }
//  }
//   /**

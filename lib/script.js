 ...
 */
  async function add(AddProp){
    ...
    const NewProperty = factory.newResource(NS , 'Property' , "1000");
    //adding property
...
/**
    ...
    * @param {org.example.empty.addTest} test - the trade to be processed
    * @transaction
    */
   async function add(test){
    const factory = getFactory();
    const NS = 'org.example.empty';
    const NewProperty = factory.newResource(NS , 'test' , "100");
    //adding property
    const PropRegistery = await getAssetRegistry(NS+'.test');
    await PropRegistery.addAll(NewProperty);
   }
